document.addEventListener('DOMContentLoaded', () => {
      ...
      for (let i = 1; i <= 4; i++) {
        letras.forEach(letra => {
          ...
          slot.innerHTML = `
            <img src="../img/${esLibre ? 'carro-verde' : 'carro-rojo'}.svg" alt="Espacio" />
  ...
  for (let i = 1; i <= 4; i++) {
    letras.forEach(letra => {
      ...
      div.className = 'slot';
      div.innerHTML = `
        <img src="../img/${libre ? 'carro-verde' : 'carro-rojo'}.svg" alt="Estado" />
        <p class="fw-semibold">${id}</p>
      `;
      if (libre) {
  ...
  espacios.forEach(id => {
    ...
    col.innerHTML = `
      <div class="card-parqueo ${libre ? 'libre' : 'ocupado'}" data-espacio="${id}" ${libre ? `onclick="seleccionarEspacio('${id}')"` : ''}>
        <img src="../img/${libre ? 'carro-verde' : 'carro-rojo'}.svg" alt="${libre ? 'Libre' : 'Ocupado'}" />
