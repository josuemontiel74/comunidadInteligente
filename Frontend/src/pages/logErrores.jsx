...
function LogErrores() {
  ...
  useEffect(() => {
    ...
    if (userGuardado) {
      try { setUsuario(JSON.parse(userGuardado)); }
      catch { navigate("/"); }
    }
  }, [navigate]);
  ...
  useEffect(() => { setPaginaActual(1); }, [busqueda, filtroNivel, filtroDesde, filtroHasta]);
  ...
  if (loading && registros.length === 0) {
    return (
      <div className="le-loading-screen">
        <div className="spinner-border" role="status" style={{ color: "#ef4444" }}>
          <span className="visually-hidden">Cargando...</span>
        </div>
        <p className="mt-3 fw-semibold" style={{ color: "#ef4444" }}>Cargando log de errores...</p>
      </div>
    );
  }
  ...
  return (
    <div className="le-dashboard">
      {/* OVERLAY */}
      <div className={`le-overlay ${menuOpen ? "active" : ""}`} onClick={() => setMenuOpen(false)} />

      {/* DRAWER */}
      <aside className={`le-drawer ${menuOpen ? "open" : ""}`}>
        <div className="le-drawer-header">
          <div className="le-drawer-avatar"><i className="bi bi-shield-exclamation"></i></div>
          <h4 className="le-drawer-title">Log de Errores</h4>
          <span className="le-drawer-user">{usuario?.username || "SuperAdmin"}</span>
        </div>

        <div className="le-drawer-body">
          <div className="le-menu-section">
            <h6 className="le-menu-section-title">Navegación</h6>
            <Link className="le-menu-item" to="/Superadmin" onClick={() => setMenuOpen(false)}>
          ...
          <div className="le-menu-section">
            ...
            <Link className="le-menu-item" to="/Auditorias" onClick={() => setMenuOpen(false)}>
              <i className="bi bi-journal-text"></i><span>Auditorías</span>
              <i className="bi bi-chevron-right le-menu-arrow"></i>
            </Link>
            <Link className="le-menu-item active" to="/LogErrores" onClick={() => setMenuOpen(false)}>
              <i className="bi bi-bug"></i><span>Log de Errores</span>
              <i className="bi bi-chevron-right le-menu-arrow"></i>
            </Link>
            <Link className="le-menu-item" to="/Reportes" onClick={() => setMenuOpen(false)}>
              <i className="bi bi-bar-chart"></i><span>Reportes</span>
              <i className="bi bi-chevron-right le-menu-arrow"></i>
            </Link>
            <Link className="le-menu-item" to="/GestionUsuario" onClick={() => setMenuOpen(false)}>
              <i className="bi bi-person-gear"></i><span>Gestión Usuarios</span>
              <i className="bi bi-chevron-right le-menu-arrow"></i>
            </Link>
          </div>
        </div>
      ...
      <div className="le-main">
        ...
        <header className="le-header">
          <button className="le-header-btn" onClick={() => navigate(-1)} title="Volver">
          ...
          <div className="le-header-actions">
            <button className="le-header-btn" onClick={cargarDatos} disabled={loading} title="Actualizar">
              <i className={`bi ${loading ? "bi-hourglass-split" : "bi-arrow-clockwise"}`}></i>
            </button>
            <button className="le-header-btn" onClick={() => setMenuOpen(true)} title="Menú">
        ...
        {error && (
          <div className="le-empty-container">
            <i className="bi bi-exclamation-triangle-fill le-empty-icon" style={{ color: "#ef4444" }}></i>
        ...
        {!error && (
          <>
            ...
            <div className="le-stats-row">
              <div className="le-stat-card" style={{ "--le-stat-color": "#ef4444" }}>
              ...
              <div className="le-stat-card" style={{ "--le-stat-color": "#f97316" }}>
                <i className="bi bi-exclamation-triangle-fill le-stat-icon"></i>
                <div className="le-stat-value">{getCount("WARN") + getCount("WARNING")}</div>
                <div className="le-stat-label">Advertencias</div>
              </div>
              <div className="le-stat-card" style={{ "--le-stat-color": "#3b82f6" }}>
              ...
              <div className="le-stat-card" style={{ "--le-stat-color": "#6b7280" }}>
            ...
            <div className="le-toolbar">
              <div className="le-toolbar-top">
                ...
                <button className="le-btn-limpiar-log" onClick={handleLimpiar} title="Eliminar registros antiguos">
              ...
              <div className="le-chips">
                {["todos", "ERROR", "WARN", "INFO", "DEBUG"].map((nivel) => (
                  <button
                    key={nivel}
                    className={filtroNivel === nivel ? getChipClass(nivel) : "le-chip"}
            ...
            {registrosFiltrados.length === 0 && !loading && (
              <div className="le-empty-container">
                <i className="bi bi-check-circle le-empty-icon" style={{ color: "#22c55e" }}></i>
                <h5>Sin registros</h5>
                <p className="text-muted">No se encontraron errores con los filtros aplicados</p>
              </div>
            )}
                  ...
                  <tbody>
                    {paginados.map((r, idx) => (
                      <tr key={r.idLog}>
                        ...
                        <td>{r.username || <span style={{ color: "#9ca3af" }}>—</span>}</td>
                        <td>
                          <span className="le-ruta">{r.rutaAfectada || "—"}</span>
                        </td>
                        <td>
                        ...
                        <td>
                          <button className="le-btn-ver" onClick={() => setDetalleRegistro(r)}>
                ...
                {paginados.map((r) => (
                  <div key={r.idLog} className="le-card">
                    <div className="le-card-header">
                      <span className={getBadgeClass(r.nivel)}>
                        <i className={`bi ${getIconNivel(r.nivel)} me-1`}></i>{r.nivel}
                      </span>
                      <span className="le-card-fecha">{formatearFecha(r.fechaHora)}</span>
                    </div>
                    <div className="le-card-body">
                      <div className="le-card-row">
                        ...
                        <div>
                          ...
                          <div className="le-card-value" style={{ fontFamily: "monospace", fontSize: "12px" }}>
                            {r.rutaAfectada || "—"}
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="le-card-actions">
                      <button className="le-card-btn ver" onClick={() => setDetalleRegistro(r)}>
                  ...
                  <nav>
                    <ul className="le-pagination">
                      ...
                      {getPaginasVisibles().map((num) => (
                        <li key={num} className={paginaActual === num ? "active" : ""}>
                          <button onClick={() => setPaginaActual(num)}>{num}</button>
                        </li>
                      ))}
      ...
      {/* MODAL DETALLE */}
      {detalleRegistro && (
        <div className="le-modal-overlay" onClick={() => setDetalleRegistro(null)}>
          <div className="le-modal" onClick={(e) => e.stopPropagation()}>
            <div className="le-modal-header">
              <i className="bi bi-bug-fill" style={{ color: "#b91c1c", fontSize: "22px" }}></i>
              <span className="le-modal-title">Detalle del Error</span>
              <button className="le-modal-close" onClick={() => setDetalleRegistro(null)}>
            ...
            <div className="le-modal-body">
              <div className="le-detail-row">
                <div className="le-detail-icon"><i className="bi bi-calendar-event"></i></div>
                <div>
                  <div className="le-detail-label">Fecha y Hora</div>
                  <div className="le-detail-value">{formatearFecha(detalleRegistro.fechaHora)}</div>
                </div>
              </div>

              <div className="le-detail-row">
                <div className="le-detail-icon"><i className={`bi ${getIconNivel(detalleRegistro.nivel)}`}></i></div>
                <div>
                  <div className="le-detail-label">Nivel</div>
                  <div className="le-detail-value">
                    <span className={getBadgeClass(detalleRegistro.nivel)}>{detalleRegistro.nivel}</span>
                  </div>
                </div>
              </div>
              ...
              {detalleRegistro.username && (
                <div className="le-detail-row">
                  <div className="le-detail-icon"><i className="bi bi-person-circle"></i></div>
                  <div>
                    <div className="le-detail-label">Usuario</div>
                    <div className="le-detail-value">{detalleRegistro.username}</div>
                  </div>
                </div>
              )}

              <div className="le-detail-row">
                <div className="le-detail-icon"><i className="bi bi-link-45deg"></i></div>
                <div>
                  <div className="le-detail-label">Ruta / Módulo</div>
                  <div className="le-detail-value" style={{ fontFamily: "monospace", fontSize: "13px" }}>
              ...
              <div className="le-detail-row">
                <div className="le-detail-icon"><i className="bi bi-chat-text"></i></div>
                <div>
                  <div className="le-detail-label">Mensaje de Error</div>
                  <div className="le-detail-value">{detalleRegistro.mensajeError}</div>
                </div>
              </div>

              {detalleRegistro.stackTrace && (
                <div className="le-detail-row">
                  <div className="le-detail-icon"><i className="bi bi-terminal"></i></div>
                  <div style={{ flex: 1 }}>
                    <div className="le-detail-label">Stack Trace</div>
                    <pre className="le-stack-box">{detalleRegistro.stackTrace}</pre>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="le-modal-footer">
              <button className="le-btn-cerrar" onClick={() => setDetalleRegistro(null)}>
                Cerrar
              </button>
            </div>
          </div>
